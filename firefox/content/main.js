...
function walkTree(root,budget){if(budget==null)budget=1000;if(!root)return budget;const st=[root];while(st.length){const nd=st.pop();if(!nd)continue;const out=[];if(nd.nodeType===1||nd.nodeType===11){budget=walk(nd,budget,out);if(budget<=0)return budget;for(let i=0;i<out.length;i++){obsShadow(out[i].shadowRoot,out[i]);st.push(out[i].shadowRoot)}}else if(nd.nodeType===3&&nd.parentElement)procText(nd)}return budget}
...
function schedule(){if(cool||queued||timer)return;timer=setTimeout(()=>{timer=0;queued=!0;idle(flush)},120)}
...
function onMut(muts){if(!on||cool)return;if(muts.length>80){trip();return}let w=!1;for(let i=0;i<muts.length;i++){const m=muts[i];if(m.type!=='childList')continue;const a=m.addedNodes;for(let j=0;j<a.length;j++){const n=a[j];if(n.nodeType===1){markDirty(n);w=!0;if(n.shadowRoot)obsShadow(n.shadowRoot,n)}else if(n.nodeType===3&&n.parentElement){markDirty(n.parentElement);w=!0}}}if(w)schedule()}
...
const shadowObs=[];
function startObs(){if(mo)return;const t=document.body||document.documentElement;if(!t)return;mo=new MutationObserver(onMut);try{mo.observe(t,{subtree:!0,childList:!0})}catch(_){}}
...
function activate(){if(on)return;on=!0;coolLv=0;trips=[];applyFont();const r=document.body||document.documentElement;if(r){dirty.add(r);queued=!0;idle(flush)}attInput();startObs()}
function deactivate(){if(!on)return;on=!1;stopObs();detInput();if(timer){clearTimeout(timer);timer=0}if(coolT){clearTimeout(coolT);coolT=0}cool=!1;queued=!1;dirty.clear();try{const els=document.querySelectorAll('.dynrtl-rtl,.dynrtl-rtl-input,.dynrtl-rtl-input-aligned');for(let i=0;i<els.length;i++){els[i].classList.remove('dynrtl-rtl');els[i].classList.remove('dynrtl-rtl-input');els[i].classList.remove('dynrtl-rtl-input-aligned')}}catch(_){}const cs=document.getElementById('dynrtl-custom-font-style');if(cs)cs.remove()}
...
loadS().then(()=>{if(document.body){idle(boot);return}document.addEventListener('DOMContentLoaded',()=>idle(boot),{once:!0});try{const o=new MutationObserver(()=>{if(document.body){o.disconnect();idle(boot)}});o.observe(document.documentElement,{childList:!0})}catch(_){}});
